...
class CustomBatchSampler(BatchSampler):
    def __init__(self, dataset, batch_size, drop_last=False):
        ...
        self.text_indices = [i for i in range(len(dataset)) if dataset.get_data_type(i) == 'edit']
        self.subject_edit_indices = [i for i in range(len(dataset)) if dataset.get_data_type(i) != 'edit']

    def __iter__(self):
        random.shuffle(self.text_indices)
        random.shuffle(self.subject_edit_indices)

        text_iter = iter(self.text_indices)
        subject_edit_iter = iter(self.subject_edit_indices)

        while True:
            batch = []
            num_text = round(self.batch_size * 0.5)
            num_subject_edit = self.batch_size - num_text

            try:
                batch.extend([next(text_iter) for _ in range(num_text)])
                batch.extend([next(subject_edit_iter) for _ in range(num_subject_edit)])
    ...
    def __len__(self):
        num_batches = len(self.subject_edit_indices) // round(self.batch_size * 0.5)
        return num_batches
