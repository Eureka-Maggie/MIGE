...
def load_checkpoint(checkpoint,
    ...
    if 'x_embedder.proj.weight' in state_dict: #原始:[1152,4,2,2]
        old_weight = state_dict['x_embedder.proj.weight']
        old_in_channels = old_weight.shape[1]
        new_in_channels = model.x_embedder.proj.in_channels
        if new_in_channels > old_in_channels:
            new_weight = torch.zeros(
                (old_weight.shape[0], new_in_channels, old_weight.shape[2], old_weight.shape[3]),
