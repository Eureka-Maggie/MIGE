    ...
    def add_params(self, params, module, prefix='', is_dcn_module=None):
        ...
        for name, param in module.named_parameters(recurse=False):
            ...
            if bypass_duplicate and self._is_in(param_group, params):
                print(f"Skipping parameter (already added): {name}")
                logger = get_root_logger()
        ...
        for child_name, child_mod in module.named_children():
            #print(f"Recursing into child module: {child_name}")
...
def build_optimizer(model, optimizer_cfg):
    ...
    optimizer_cfg.setdefault('constructor', 'MyOptimizerConstructor') #{'type': 'AdamW', 'lr': 2.8284271247461906e-05, 'weight_decay': 0.03, 'eps': 1e-10, 'constructor': 'MyOptimizerConstructor'}
    ...
    paramwise_cfg = Config(dict(cfg=dict(custom_keys=custom_keys))) #Config (path: None): {'cfg': {'custom_keys': {}}}
    if given_cfg := optimizer_cfg.get('paramwise_cfg'): #no
        paramwise_cfg.merge_from_dict(dict(cfg=given_cfg))
    optimizer_cfg['paramwise_cfg'] = paramwise_cfg.cfg # {'type': 'AdamW', 'lr': 2.8284271247461906e-05, 'weight_decay': 0.03, 'eps': 1e-10, 'constructor': 'MyOptimizerConstructor', 'paramwise_cfg': {'custom_keys': {}}}
