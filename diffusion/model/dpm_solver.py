...
def model_wrapper(
    ...
    def noise_pred_fn(x, t_continuous, state = None, cond=None):
        t_input = get_model_input_time(t_continuous)
        if cond is None:
            output = model(x, t_input, **model_kwargs)
        else:
            output = model(x, t_input, cond, state = state, **model_kwargs) ###
        if model_type == "noise": ###
            return output
    ...
    def model_fn(x, t_continuous):
        ...
        if guidance_type == "uncond":
            ...
            return noise - guidance_scale_y * expand_dims(sigma_t, x.dim()) * cond_grad
        elif guidance_type == "classifier-free": ###
            if unconditional_condition == "subject":
                x_in = torch.cat([x] * 2) #[10,4,64,64]
                t_in = torch.cat([t_continuous] * 2)
                c_in = condition
                noise_uncond, noise = noise_pred_fn(x_in, t_in, cond=c_in, state = "subject").chunk(2)
                return noise_uncond + guidance_scale_y * (noise - noise_uncond)
            elif unconditional_condition == "edit":
                x_in = torch.cat([x] * 3) #[10,4,64,64]
                t_in = torch.cat([t_continuous] * 3)
                c_in = condition
                noise_uncond_sy, noise_uncond_y, noise = noise_pred_fn(x_in, t_in, cond=c_in, state = "edit").chunk(3)
                return noise_uncond_sy + guidance_scale_s * (noise_uncond_y-noise_uncond_sy) + guidance_scale_y * (noise - noise_uncond_y)
...
class DPM_Solver:
    ...
    def multistep_dpm_solver_update(self, x, model_prev_list, t_prev_list, t, order, solver_type='dpmsolver'):
        ...
        """
        if order == 1:
            return self.dpm_solver_first_update(x, t_prev_list[-1], t, model_s=model_prev_list[-1]) #x:[bs,4,64,64]
        elif order == 2:
            ...
            if method == 'adaptive':
                ...
                for step in range(1, order): ###
                    ...
                    x = self.multistep_dpm_solver_update(x, model_prev_list, t_prev_list, t, step,
                                                         solver_type=solver_type)
                    if self.correcting_xt_fn is not None:#no
                        x = self.correcting_xt_fn(x, t, step)
                    if return_intermediate:#no
                        intermediates.append(x)
