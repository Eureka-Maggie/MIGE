...
class GaussianDiffusion:
    ...
    def q_sample_kl(self, x_start, t, noise=None):
    ...
    def training_losses(self, model, x_start, timestep, model_kwargs=None, noise=None, skip_noise=False):
        ...
        if skip_noise:
            ...
            if noise is None:
                noise = th.randn_like(x_start) #x_start.size()->[512,4,32,32] [bs,4,64,64]
            x_t = self.q_sample(x_start, t, noise=noise) #[bs,4,64,64]
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model(x_t, t, **model_kwargs) ##dict_keys(['y', 'ref', 'data_info']) 
            if isinstance(model_output, dict) and model_output.get('x', None) is not None:
                output = model_output['x']
            else: #true
                output = model_output

            if self.return_startx and self.model_mean_type == ModelMeanType.EPSILON: #no
            ...
            if self.model_var_type in [
                ...
                if self.loss_type == LossType.RESCALED_MSE:#no
            ...
            if model_kwargs.get('mask_ratio', False) and model_kwargs['mask_ratio'] > 0:#false
